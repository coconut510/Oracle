...
BEGIN
    SELECT EMP_NAME , NVL((SELECT DEPT_TITLE FROM DEPARTMENT WHERE E.DEPT_CODE = DEPT_ID),'부서없음') 
    INTO EMP.EMP_NAME, DEP.DEPT_TITLE
    FROM EMPLOYEE E
    WHERE EMP_ID = '&사원번호' ;  
    ...
    SELECT EMP_NAME, SALARY, NVL(DEPT_TITLE,'부서없음')
    INTO NAME, SALARY, DEPT
    FROM EMPLOYEE E LEFT JOIN DEPARTMENT D  ON (NVL(E.DEPT_CODE,'없음') = NVL(D.DEPT_ID,'없음'))
    WHERE SALARY = (SELECT MAX(SALARY) FROM EMPLOYEE E1
                        WHERE NVL(E.DEPT_CODE,'없음') = NVL(E1.DEPT_CODE,'없음')
                        GROUP BY DEPT_CODE)
         AND NVL(DEPT_TITLE,'부서없음') = NVL(DEP.DEPT_TITLE,'부서없음');
...
/
    ...
    FROM EMPLOYEE E
    WHERE EMP_ID = '&사원번호' ;
    HAVING (NVL(E.DEPT_CODE,'없음') = NVL(D.DEPT_ID,'없음');
...
BEGIN
    ...
    FROM(SELECT EMP_NAME,SALARY, DEPT_TITLE
    FROM EMPLOYEE
    ...
    IF(FIRST_SALARY BETWEEN 1000000 AND 1990000)
    THEN DBMS_OUTPUT.PUT_LINE('저임금');
    ELSIF (FIRST_SALARY BETWEEN 2000000 AND 2990000)
    THEN DBMS_OUTPUT.PUT_LINE('평균임금');
    ELSIF (FIRST_SALARY BETWEEN 3000000 AND 3990000)
    THEN DBMS_OUTPUT.PUT_LINE('고임금');
    ELSIF (FIRST_SALARY BETWEEN 4000000 AND 4990000)
    THEN DBMS_OUTPUT.PUT_LINE('능력자');
    ELSE DBMS_OUTPUT.PUT_LINE('CEO');
    END IF;
END;
/
